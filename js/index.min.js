...
;(function(){var elInput=doc.get("elInput");var elOutput=doc.get("elOutput");var elBox=doc.get("elBox");var btnDownload=doc.get("btnDownload");var lastData="";var Event=doc.Event;var debounce=function debounce(func,wait){var timeout=void 0,args=void 0,context=void 0,timestamp=void 0;return function _(){context=this;for(var _len=arguments.length,_args=Array(_len),_key=0;_key<_len;_key++){_args[_key]=arguments[_key]}args=_args;timestamp=Date.now();var later=function _later(){var last=Date.now()-timestamp;if(last<wait){timeout=setTimeout(later,wait-last)}else{timeout=null;func.apply(context,args)}};if(!timeout){timeout=setTimeout(later,wait)}}};var getWindowSize=function getWindowSize(){var width=0;var height=0;if(window.innerWidth){width=window.innerWidth;height=window.innerHeight}else if(document.documentElement&&document.documentElement.clientWidth){width=document.documentElement.clientWidth;height=document.documentElement.clientHeight}else if(document.body){width=document.body.clientWidth;height=document.body.clientHeight}return{width:width,height:height}};var addColon=function addColon(s){if(s.includes(":")){return s}return s+" : "};var check=function check(s){var x=Bella.trim(s);if(!x.length){return""}var a=x.split(/\n/);if(!a.length){return""}var b=a.map(addColon);x=b.join("\n");return x};var updateDownloadLink=function updateDownloadLink(svg){var xml=['<?xml version="1.0" encoding="utf-8" standalone="no"?>','<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 20010904//EN" "http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd">','<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',svg.innerHTML,"</svg>"].join("");var title="Untitled Sequence Diagram";var a=lastData.split("\n");var m=a[0];var n=m.toLowerCase();if(n.startsWith("title")&&n.includes(":")){var b=m.indexOf(":");title=Bella.trim(m.substr(b+1,m.length))}var fname=Bella.createAlias(title);btnDownload.onclick=function(){svg.focus();saveAs(new Blob([xml],{type:"application/image/svg+xml;charset=utf-8"}),fname+".svg")}};var updatePosition=function updatePosition(svg){var w=parseInt(svg.getAttribute("width"),10);var h=parseInt(svg.getAttribute("height"),10);var ws=getWindowSize();var ww=ws.width;var wh=ws.height-70;var sw=Math.max(ww,w);var sh=Math.max(wh,h);svg.setAttribute("width",sw);svg.setAttribute("height",sh)};var updateDragger=function updateDragger(){var svg=elOutput.querySelector("svg");svgPanZoom(svg,{viewportSelector:"#elOutput",zoomEnabled:true,controlIconsEnabled:false,dblClickZoomEnabled:false,contain:false,fit:false,center:true});updatePosition(svg);updateDownloadLink(svg)};var renderOutput=function renderOutput(v){try{var diagram=Diagram.parse(v);elOutput.empty();diagram.drawSVG(elOutput);lastData=v;updateDragger();return diagram}catch(e){return e}};var debouncedRenderOutput=debounce(renderOutput,1e3);Event.on(elInput,"keyup",function(){var v=check(elInput.value);if(!v.length||v===lastData){return false}return debouncedRenderOutput(v)});var initSample=function initSample(){var v=["Title: The process of service","Bob -> Alice : request service","Alice -> John : ask for help","Alice -> Nina : verify","John -> Tom : ask key","Nina -> Alice : return result","Note right of Tom : Tom does not remember","Tom --> Tom : ask himself","Tom --> Nina : ask for a hint","Note left of Nina : Nina forgot too","Nina --> Tom : reject","Tom --> Kelly : ask for a hint","Kelly -->> Tom : hint","Tom --> Alice : share key","Alice -->> Bob : reply"].join("\n");elInput.value=v;renderOutput(v);var handler=elBox.querySelector(".drag-handler");draggable(elBox,handler)};initSample()}());